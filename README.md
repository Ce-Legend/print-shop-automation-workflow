...
它围绕真实门店动作拆了一套 Windows 工作流。对我来说，最有价值的地方在于：线下操作里那些重复判断，最后变成了可检查、可恢复、可交接的流程。
...
| 输入文件夹名 | 解析出的任务 | 队列动作 |
|---|---|---|
| `【拍立得】5寸,10张 250701-123456789012345` | 5寸、拍立得、10张、订单号 | 图片预处理后使用 `5寸拍立得` 预设 |
| `【全景】6寸,8张 250702-123456789012346` | 6寸、全景、8张、订单号 | 跳过拍立得预处理，使用 `6寸全景` 预设 |
| `3T_拍立得_5张250703-123456789012347` | 3寸、拍立得、5张、订单号 | 使用默认小尺寸预设，进入打印队列 |

如果文件夹名缺少尺寸或订单号，系统会把它归为异常任务；如果只缺少模式，会继续使用该尺寸的默认预设。
...
打印店订单本来就以文件夹流转。把尺寸、模式、张数、订单号放进文件夹名，比额外维护一张表更贴近现场。这里的 `FolderNameParser` 支持中文尺寸、数字别名、张数和订单号，就是为了降低一线使用门槛。

### 打印机预设要承认边界

Windows 标准打印 API 能控制一部分基础参数，但很难完整接管厂商驱动里的高级色彩预设。这个项目采用一次人工配置、后续自动复用的办法。技术上不完美，但对门店交付更务实。

### 异常和首次配置都要给普通用户看得懂

线下系统出错时，没人会一直盯控制台。项目里同时做了日志、异常文件夹、界面状态和语音提示；另外还有预设配置引导和配置状态检查工具。照片格式不支持、文件夹命名不完整、打印机不可用、首次配置没完成，都应该变成店员能理解的状态。
